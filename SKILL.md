...
- **Tier 1 — 2D dimensioned drawings** (`visualize` / `show_widget`, SVG): scaled
  plan / front-elevation / side-elevation / section with real dimension lines.
  Settle layout and proportions here; cheapest, iterate freely.
- **Tier 2 — realistic 3D preview** (three.js product render): once the layout is
  roughly right, show "how it could look" — orbitable, real materials, soft
  shadows, studio lighting — so the user signs off on form, material, and feel.
  This runs **whether or not SketchUp is connected**, and is the approval gate.
  There is no AI photoreal generator here, so this render is the realistic
  preview; aim for product-render quality (see `references/visualization.md`).
...
Keep one structured spec for the project as you go — overall envelope, parts with
their parametric dimensions, material, joinery, hardware, and the measurement
sources. It is the single source of truth that the visualizations, the script,
and the drawings all read from. When in doubt, update the spec, then regenerate
the views and the cut list from it, so they can never drift apart. The JSON shape
the script consumes is in `references/deliverables.md`.
